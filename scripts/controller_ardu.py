...
class Flight_controller:
    def __init__(self, cv = False):
        ...
        self.get_pose_globle_local = rospy.Subscriber('/mavros/global_position/local', Odometry, self.get_pose_globle)
    ...
    def ml_callback(self, data):
        ...
        self.mast_xyz = np.array([data.x, data.y, data.z])
    ...
    def takeoff(self, t_alt):
        ...
        try:
            ...
            self.takeoff_service(0.0,0.0,0.0, 0.0, t_alt)
        except rospy.ServiceException as e:
            rospy.loginfo("Service call failed: " %e)
    ...
    def set_offboard_mode(self):
        ...
        try:
            self.flight_mode_service(0, 'GUIDED')
    ...
    def get_pose_globle(self, location_data):
        self.curr_x = location_data.pose.pose.position.x
        self.curr_y = location_data.pose.pose.position.y
        self.curr_z = location_data.pose.pose.position.z
        self.curr_ori_x = location_data.pose.pose.orientation.x
        self.curr_ori_y = location_data.pose.pose.orientation.y
        self.curr_ori_z = location_data.pose.pose.orientation.z
        self.curr_ori_w = location_data.pose.pose.orientation.w
        rot_q = location_data.pose.pose.orientation
        self.curr_roll ,self.curr_pitch ,self.curr_yaw= quad_eular(rot_q.x, rot_q.y, rot_q.z, rot_q.w)
    ...
    def move_wrtDrone_fixedaxis(self, dx, dy, dz):
    ...
    def move_wrt_drone(self, dx, dy, dz = 0):
        yaw = math.radians(self.curr_yaw)
        ...
        self.set_x = self.curr_x + new_dx
        self.set_y = self.curr_y + new_dy
        self.set_z = 1.2


if __name__ == '__main__':
    ct = Flight_controller()
    ct.toggle_arm(True)
    time.sleep(1)
    ct.set_offboard_mode()
    ct.takeoff(2)
    time.sleep(3)
    # ct.set_waypoint(2,2,1.5)
    # ct.set_orientation(0,0,45)
    ct.d_yaw(0)
    # ct.move_wrt_drone(0, -10, 1.5)
    # ct.d_yaw(135)
    ct.set_pose()
    #time.sleep(3)
    # ct.set_waypoint(10, -5, 5)
    # ct.set_pose()
    # ct.set_waypoint(0,0,5)
    # ct.set_pose()
