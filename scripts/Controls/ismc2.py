...
if __name__ == '__main__':
	...
	pickle_off = open("traj_6f.txt", "rb")			#modify the first input in open() to be the location of the trajectory file, and default is in home
	...
	ax_vy.plot3D(x_path, y_path, y_ddot_path, linestyle = '-', marker = '.', color = 'teal')
	ax_vy.plot3D(x_true, y_true, y_ddot_true, linestyle = '-', color = 'lime')

	ax_vy.set_title('Acceleration in $y$').set_fontsize(20)
	ax_vy.set_xlabel('$x$').set_fontsize(20)
	ax_vy.set_ylabel('$y$').set_fontsize(20)
	ax_vy.set_zlabel('$a_y$').set_fontsize(20)
	ax_vy.axes.set_xlim3d(left=-10, right=10)
	plt.legend(['Planned Path','Executed Path','Desired $a_y$','Executed $a_y$'], fontsize = 14)
